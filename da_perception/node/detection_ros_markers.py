...
class DetectionBoxMarker:
    ...
    def get_ros_axes_markers(self, axes_scale=1.0):
        ...
        if self.z_axis is not None:
            ...
            name = base_name = "_z_axis"
            markers.append(
                self.create_axis_marker(self.z_axis, id_num, rgba, name, axes_scale=axes_scale)
            )
        if self.x_axis is not None:
            ...
            markers.append(
                self.create_axis_marker(self.x_axis, id_num, rgba, name, axes_scale=axes_scale)
            )
        if self.y_axis is not None:
            ...
            markers.append(
                self.create_axis_marker(self.y_axis, id_num, rgba, name, axes_scale=axes_scale)
            )
    ...
    def update(self, detections_3d, timestamp=None):
        ...
        for detection_3d in detections_3d:
            ...
            if box_3d is not None:
                ...
                marker_label = self.default_marker_base_name if label is None else label

                new_marker = DetectionBoxMarker(self.detection_box_id, marker_label)
                self.collection[self.detection_box_id] = new_marker
                self.collection[self.detection_box_id].update(
